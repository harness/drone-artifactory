...
Write-Host "+ docker build -f docker/Dockerfile.windows.amd64.${env:VERSION} -t plugins/artifactory:windows-${env:VERSION}-amd64 .";
docker build -f docker/Dockerfile.windows.amd64.${env:VERSION} -t plugins/artifactory:windows-${env:VERSION}-amd64 .
Write-Host "+ docker push plugins/artifactory:windows-${env:VERSION}-amd64"
docker push plugins/artifactory:windows-${env:VERSION}-amd64

# remove images from local cache
Write-Host "+ docker rmi plugins/artifactory:windows-${env:VERSION}-amd64"
docker rmi plugins/artifactory:windows-${env:VERSION}-amd64
