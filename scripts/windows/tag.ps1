...
$env:IMAGE_PATCH="plugins/artifactory:${env:DRONE_SEMVER_SHORT}-windows-${env:VERSION}-amd64"
$env:IMAGE_MAJOR="plugins/artifactory:${env:DRONE_SEMVER_MAJOR}-windows-${env:VERSION}-amd64"
$env:IMAGE_MINOR="plugins/artifactory:${env:DRONE_SEMVER_MAJOR}.${env:DRONE_SEMVER_MINOR}-windows-${env:VERSION}-amd64"
...
# build the binary
Write-Host "+ go build -o release/windows/amd64/plugin"
...
Write-Host "+ docker build -f docker/Dockerfile.windows.amd64.${env:VERSION} -t ${env:IMAGE_PATCH} ."
